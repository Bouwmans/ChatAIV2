 ...
 * - Auth: Only logged-in users (ties to Cognito).
...
const schema = a.schema({
  chat: a
    ...
    .authorization((allow) => [allow.authenticated()])  // Fixed: Simple authenticated access (no .to).
    .handler(a.handler.function(chatBedrock)),
}).authorization((allow) => [allow.authenticated()]);  // Global schema auth.
